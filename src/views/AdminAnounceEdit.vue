<template>
    <el-col>
        <el-row>
            <el-col :offset="1">
                <a-page-header title="内容编辑" subtitle="编辑公告" :show-back="true" @back="() => {
                    router.back();
                }">
                    <template #breadcrumb>
                        <a-breadcrumb>
        ...
        <el-row>
            <el-col :span="16" :offset="1">
                ...
                <el-button v-if="anounce.Status == 1" type="danger" @click="handleMakeVisible(false)"><el-icon>
                        <CircleClose />
                    </el-icon><span>&nbsp;关闭此公告访问</span></el-button>
                <el-button v-if="anounce.Status == 0" type="success" @click="handleMakeVisible(true)"><el-icon>
                        <CircleCheck />
                    </el-icon><span>&nbsp;重新打开此公告访问</span></el-button>
                <p />
                <a-space>
...
onMounted(() => {
    ...
    fetch("http://127.0.0.1:6521/api/anounce/details?id=" + aid.value, requestOptions)
        ...
        .then(result => {
            ...
            const formanounce = <FormAnounce>{
                title: anounce.value.Title,
                introduction: anounce.value.Introduction,
                content: anounce.value.Text
            };
            vFormRef.value.setFormData(formanounce);
            valueHtml.value = anounce.value.Text;
        })
...
const handleMakeVisible = (status: boolean) => {
    ...
    fetch("http://127.0.0.1:6521/api/anounce/visible", requestOptions)
        .then(response => response.text())
        .then(result => {
...
const handleAnounceSubmit = (formData: any) => {
    ...
    fetch("http://127.0.0.1:6521/api/anounce/update", requestOptions)
        .then(response => response.text())
        .then(result => {
