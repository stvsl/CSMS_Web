<template>
        ...
        <a-row>
            <a-col :span="22" :offset="0">
                <a-card title="统计数据">
                    <a-row>
                        <a-col :flex="1">
                            <a-statistic title="总反馈数" :value="overdata.TotalFeedbacks" :value-from="0" :start="true"
                                animation>
                            </a-statistic>
                        </a-col>
                        <a-col :flex="1">
                            <a-statistic title="已处理" :value="overdata.TotalProcessed" :value-from="0" :start="true"
                                animation>
                            </a-statistic>
                        </a-col>
                        <a-col :flex="1">
                            <a-statistic title="处理中" :value="overdata.TotalPending" :value-from="0" :start="true"
                                animation>
                            </a-statistic>
                        </a-col>
                        <a-col :flex="1">
                            <a-statistic title="处理完成率" :value="overdata.CompletionRate * 100" :value-from="0"
        ...
        <a-row>
            <a-col :span="14" :offset="0">
                <a-card title="反馈事件">
                    <template #extra>
                        <el-col :span="4">
                            <el-pagination :page-size="4" background layout="prev, pager, next, jumper" :total="count"
                                @change="(currentPage) => {
            ...
            <a-col :span="7" :offset="1">
                <a-card title="操作中心">
                    <a-space direction="vertical" size="medium">
                        ...
                        <a-button-group v-if="false">
                            <a-input :style="{ width: '200px' }" placeholder="请输入搜索关键字" v-model="searchKeyword"
                                allow-clear />
                            &nbsp;
                            <a-button @click="() => { }">
                                查询
    ...
    <a-modal v-model:visible="visible" :width="600" title="创建反馈事件" @cancel="handleCancel" @before-ok="handleBeforeOk"
        :simple="true">
        <a-col :flex="1">
            <div style="width: 99%;">
...
import UserFeedCard from "../usercomponents/UserFeedCard.vue";
...
const handleBeforeOk = (done) => {
    vFormRef.value.getFormData().then(formData => {
        ...
        fetch("http://127.0.0.1:6521/api/feed/upload", requestOptions)
...
onMounted(() => {
    ...
    fetch("http://127.0.0.1:6521/api/feed/user/count?id=" + userStore.ID, requestOptions)
        .then(response => response.text())
        .then(result => count.value = JSON.parse(result).data)
        .catch(error => console.log('error', error));
    handleFetchfeed(1);
    fetch("http://127.0.0.1:6521/api/feed/user/overview?id=" + userStore.ID, requestOptions)
        .then(response => response.text())
...
const handleFetchfeed = () => {
    ...
    fetch("http://127.0.0.1:6521/api/feed/user/id/list?id=" + userStore.ID + "&page=" + page.value, requestOptions)
        .then(response => response.text())
